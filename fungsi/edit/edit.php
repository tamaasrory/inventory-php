...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['pelanggan'])){
		$id= htmlentities($_POST['id']);
		$tgl= date("j F Y, G:i");
		$data[] = htmlentities($_POST['nama']);
		$data[] = htmlentities($_POST['nomor_hp']);
		$data[] = htmlentities($_POST['alamat']);
		$data[] = htmlentities($_POST['nomor_kendaraan']);
		$data[] = $tgl;
		$data[] = $id;
		$sql = 'UPDATE pelanggan SET nama=?,nomor_hp=?,alamat=?,nomor_kendaraan=?,tgl_edit=? WHERE id_pelanggan=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=pelanggan&success-edit=edit-data"</script>';
	}
	...
	if(!empty($_GET['jual'])){
		...
		$pl = htmlentities($_POST['pelanggan']);
		$idp = htmlentities($_POST['idp']);
		
		$sql_tampil = "select * from barang b left join kategori k on k.id_kategori = b.id_kategori where b.id_barang=?";
		$row_tampil = $config -> prepare($sql_tampil);
		$row_tampil -> execute(array($id_barang));
		$hasil = $row_tampil -> fetch();

		if(($hasil['stok'] > $jumlah) || ( strtolower($hasil['nama_kategori']) == 'jasa') )
		{
			...
			$row1 -> execute($data1);
			echo '<script>window.location="../../index.php?pelanggan='.$pl.'&idp='.$idp.'&page=jual#keranjang"</script>';
		}else{
			echo '<script>alert("Keranjang Melebihi Stok Items Anda !");
					window.location="../../index.php?pelanggan='.$pl.'&idp='.$idp.'&page=jual#keranjang"</script>';
		}
